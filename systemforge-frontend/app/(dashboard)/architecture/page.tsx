  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        try {
          result = JSON.parse(config.generatedOutputJson);
        } catch (parseError) {
          // Fallback for new MABA Markdown output format
          const text = config.generatedOutputJson;
          const foundModules: ModuleRecommendation[] = [];
          
          const defaultModules = [
            { id: 'AUTH', keywords: ['Authentication', 'JWT', 'OAuth', 'Auth ', 'Identity'] },
            { id: 'DATABASE', keywords: ['PostgreSQL', 'MySQL', 'Database', 'MongoDB', 'SQL'] },
            { id: 'CACHING', keywords: ['Redis', 'Memcached', 'Cache'] },
            { id: 'MESSAGING', keywords: ['Kafka', 'RabbitMQ', 'SQS', 'Message', 'Event'] },
            { id: 'PAYMENT', keywords: ['Payment', 'Stripe', 'Razorpay', 'Billing'] },
            { id: 'NOTIFICATION', keywords: ['Notification', 'Push', 'Email', 'Firebase', 'APNS'] },
            { id: 'SEARCH', keywords: ['Elasticsearch', 'Solr', 'Search'] },
            { id: 'MONITORING', keywords: ['Monitoring', 'Prometheus', 'Grafana', 'Observability', 'Datadog'] },
            { id: 'STORAGE', keywords: ['S3', 'Storage', 'Blob', 'CloudFront'] }
          ];

          defaultModules.forEach(mod => {
            if (mod.keywords.some(k => text.includes(k) || text.includes(k.toLowerCase()))) {
              foundModules.push({
                module: mod.id,
                recommendations: [{
                  title: `${mod.id} Service`,
                  description: `AI-recommended component detected in architectural specification.`,
                  confidence: 0.95
                }]
              });
            }
          ...
          result = {
            appType: config.appType || 'System',
            appScale: config.appScale || 'Scale',
            architectureSummary: 'Generated by Multi-Agent Backend Architecture (MABA)',
            modules: foundModules.length > 0 ? foundModules : [
               { module: 'ARCHITECTURE', recommendations: [{ title: 'Core Services', description: 'Main backend services', confidence: 0.99 }] }
            ]
          };
        }
